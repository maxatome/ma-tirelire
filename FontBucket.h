...
// calling FmInit.
  ...
  // MAX remplace un __inline par un define
#define FmIsInstalled(fmPtr)	(fmPtr->localID != 0)
#if 0
...
// default is off.
  ...
  // MAX remplace deux __inline par un define
#define FmPreventSonyHiRes(fmPtr, prevent) (fmPtr->preventSonyHiRes = prevent)
#define FmIsSonyHiResPrevented(fmPtr)	   (fmPtr->preventSonyHiRes)
#if 0
...
public:	
	...
	Err getIndexedFontInfo(UInt16 index, Char * fontName, FmFontInfoType* info) {
		return FmGetIndexedFontInfo(&m_params, index, info);
	}
	
	Err info (FmFontID fmFontId, FmFontInfoType* info) {
		return FmGetFontInfo(&m_params, fmFontId, info);
	}		
